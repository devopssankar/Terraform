data "aws_acm_certificate" "service" {
  ...
  domain                    = "${element(var.fqdns, count.index)}"
  statuses                  = ["${var.accepted_certificate_status}"]
}
resource "aws_elastic_beanstalk_environment" "bs" {
  ...
  setting {
    ...
    name                    = "Xms"
    value                   = "${var.xms_heap_size}"
  }
  setting {
    namespace               = "aws:elasticbeanstalk:container:tomcat:jvmoptions"
    name                    = "Xmx"
    value                   = "${var.xmx_heap_size}"
  }
  setting {
  ...
  setting {
    ...
    value                   = "${element(data.aws_acm_certificate.service.*.arn, count.index)}"
  }
  setting {
...
resource "aws_route53_record" "fqdn" {
  ...
  count                     = "${length(var.sslfqdns)}"
  zone_id                   = "${var.zone_id}"
  name                      = "${element(var.sslfqdns, count.index)}"
  type                      = "A"
  alias {
